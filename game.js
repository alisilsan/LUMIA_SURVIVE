...
// (전역 스코프를 공유하는 클래식 스크립트 방식 — file:// 로 직접 열어도 동작합니다)
    ...
    for (const f of cinematicEffects) {
      ...
      if (f.type === 'fusion' && f.life > 0) {
        for (const e of enemies) {
          ...
          if (d < f.radius && d > 8 && elapsed*1000 >= (e.moveFrozenUntil || 0)) { e.x += dx * dt * 2; e.y += dy * dt * 2; }
        }
      }
  ...
  function initGame() {
    ...
    player = {
      ...
      itemFlatDamage: 0,
      rerollsLeft: 3,
      cooldownMult: 1, // 공용 스킬과 캐릭터 고유 스킬에 적용
      magnetRadius: 90,
      ...
      statPicks: { speed: 0, hp: 0, cdr: 0, magnet: 0, guardianSuit: 0, burgundy47: 0, fateDice: 0, lightEmblem: 0 },
      jackieKillStreak: 0,
      bloodFrenzyUntil: 0,
      basicAttackTimer: 0,
      uniqueSkills: {},
  ...
  function updateWeapons(dt, now) {
    for (const wid in player.weapons) {
      ...
      if (w.cooldownTimer <= 0) {
        w.cooldownTimer += stat.cooldown * player.cooldownMult; // 공용 스킬 쿨타임 감소
  ...
  function updateUniqueSkills(dt, now) {
    ...
    for (const skillKey in player.uniqueSkills) {
      ...
      w.cooldownTimer -= dt * 1000;
      if (w.cooldownTimer <= 0) {
        // 고유 스킬에도 델루리안 타임피스 적용
        w.cooldownTimer += stat.cooldown * 1000 * player.cooldownMult;
  ...
  function effectiveSpeed(e, now) {
    if (now < (e.moveFrozenUntil || 0)) return 0;
  ...
  function shoveEnemy(e, fromX, fromY, distance) {
    if (elapsed*1000 < (e.moveFrozenUntil || 0)) return;
  ...
  function updateEnemies(dt, now) {
    for (const e of enemies) {
      ...
      if (e.isBoss) {
        const frozen = now < (e.moveFrozenUntil || 0), x=e.x, y=e.y;
        updateBossAI(e, dt, now);
        if (frozen) { e.x=x; e.y=y; } // 공격/패턴 타이머는 유지
      } else {
  ...
  function updateBossAI(e, dt, now) {
    ...
    if (e.bossType === 'weakline') {
      if (e.ai.dashState === 'dashing') {
        moveOverridden = true;
        const dashSpeed = now < (e.moveFrozenUntil || 0) ? 0 : 620;
  ...
  function updateHazardZones(dt) {
    ...
    for (const z of hazardZones) {
      ...
      if (z.jackieBloodZone) {
        z.tickTimer -= dt;
        while (z.tickTimer <= 1e-9 && z.ticksLeft > 0) {
    ...
    hazardZones = hazardZones.filter((z) => z.jackieBloodZone ? z.ticksLeft > 0 : z.life > 0);
  }
  ...
  function damageEnemy(e, amount, options = {}) {
    if (e.dead) return;
    if (options.scale !== false && player.damageMult && player.damageMult !== 1) amount *= player.damageMult;
  ...
  function handleJackieBloodFestivalKill() {
    ...
    if (player.jackieKillStreak >= 50) {
      ...
      spawnPulse(player.x,player.y,75,CHARACTERS.jackie.color,'ring');
    }
  }
  ...
  function applyConsumableItem(key) {
    ...
    if (key === 'cubeWatch') {
      for (const e of enemies) {
        if (!e.dead && Math.abs(e.x-player.x) <= W/2+e.radius && Math.abs(e.y-player.y) <= H/2+e.radius) {
          e.moveFrozenUntil = elapsed*1000 + 3000;
        }
      }
      spawnPulse(player.x,player.y,Math.min(W,H)/2,'#79dbff','ring');
    } else if (key === 'greatPill') {
      player.maxHp += 50; player.hp += 50;
    } else if (key === 'seleneTear') {
      player.itemFlatDamage += 5;
    }
  ...
  function xpGainMultiplier() {
    // 10레벨마다 경험치 획득량 +5%
    return 1 + Math.floor(player.level / 10) * 0.05;
  }
  ...
  function drawTacticalHUD() {
    ...
    {
      const size = compact ? 88 : 142, mx = W - size - 20, my = compact ? 155 : 90;
  ...
  function render() {
    ...
    if (isJackieFrenzyActive(now)) {
      ctx.save();ctx.strokeStyle=CHARACTERS.jackie.color;ctx.globalAlpha=.45+.2*Math.sin(elapsed*8);ctx.lineWidth=3;
      ctx.beginPath();ctx.ellipse(px,py+15,27+Math.sin(elapsed*6)*3,12,0,0,Math.PI*2);ctx.stroke();ctx.restore();
    }
